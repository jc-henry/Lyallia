...
aus25_pcr1_df <- read.table('../deseq2_1.5FC/aus25_vs_pcr1/agrigo_nopadjust_aus25_pcr1_downreg.txt', header = T, sep = '\t') %>% 
  filter(pvalue < 0.05)
aus25_pcr2_df <- read.table('../deseq2_1.5FC/aus25_vs_pcr2/agrigo_nopadjust_aus25_pcr2_downreg.txt', header = T, sep = '\t') %>% 
  filter(pvalue < 0.05)
aus30_pcr1_df <- read.table('../deseq2_1.5FC/aus30_vs_pcr1/agrigo_nopadjust_aus30_pcr1_downreg.txt', header = T, sep = '\t') %>% 
  filter(pvalue < 0.05)
aus30_pcr2_df <- read.table('../deseq2_1.5FC/aus30_vs_pcr2/agrigo_nopadjust_aus30_pcr2_downreg.txt', header = T, sep = '\t') %>% 
  filter(pvalue < 0.05)
...
write.table(lst_df, 'downreg_GO_venn_intersections.txt', col.names = F, row.names = F, quote = F, sep = '\t')


### make a good-looking venn diagram
library("ggvenn")
pdf('downreg_GO_venn.pdf')
